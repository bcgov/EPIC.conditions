...
def _build_page_chunk_prompt(doc_type: str, section_headers: List[str], label: str, page_text: str) -> str:
    ...
    if doc_type == "table_format":
        ...
        return (
            ...
            f"condition_name MUST combine the row's own Ref # together with its Subject Area heading, "
            f"formatted exactly as \"<Ref #> - <Subject Area heading>\" (e.g., \"A1 - Mine Design, "
            f"Development and Operation\", \"C12 - Wildlife & Terrestrial Resources\"). The same Subject "
            f"Area heading applies to every row underneath it until a new Subject Area appears. Never use "
            f"just the Ref # alone, and never use just the Subject Area heading alone — condition_name must "
            f"always have both parts, in that order, separated by \" - \". Do NOT create a condition for a "
            f"Subject Area heading row that has no commitment text of its own — only create a condition for "
            f"rows that have an actual Ref # and commitment. "
